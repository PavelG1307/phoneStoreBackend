pipeline {
  ...
  post {
     success { 
        ...
        sh ("""GIT_MESSAGE=\$(git log -1 ${env.GIT_COMMIT}) && curl -s -X POST https://api.telegram.org/bot$TOKEN/sendMessage -d chat_id=$CHAT_ID -d parse_mode=markdown -d text='Ура! Новая сборка!✅\nКоммит: \$GIT_MESSAGE\nОт: ${env.GIT_COMMITTER_NAME}\nРепозиторий: ${env.JOB_NAME}\nВетка: ${env.GIT_BRANCH}'""")
        }
     }
